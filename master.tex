...
% \begin{enumerate}[leftmargin=46pt]
% \renewcommand{\labelenumi}{(\theenumi)}
% \item 画出异步电动机T形等效电路和简化等效电路。
% \item 求额定运行时的转差率$s_{\mathrm{N}}$，定子额定电流$I_{\mathrm{sN}}$和额定电磁转矩。
% \item 定子电压和频率均为额定值时，理想空载时的励磁电流$I_0$。
% \item 定子电压和频率均为额定值时，临界转差率$s_{\mathrm{m}}$和临界转矩$T_{\mathrm{m}}$，并画出该异步电机的机械特性。
% \end{enumerate}
